...
class _BaseAdapter(abc.ABC):
    """Single-format adapter used by :class:`DataHandle`."""
    ...
    def open(self, table: pa.Table):
        """Yield a file-like view of *table* for the adapter's format."""
...
SUPPORTED_FORMATS: tuple[str, ...] = tuple(adapter.format for adapter in _ADAPTERS)
_ADAPTER_BY_FORMAT: Mapping[str, _BaseAdapter] = {adapter.format: adapter for adapter in _ADAPTERS}
...
def _slice_table(table: pa.Table, page_size: int, page: int | None) -> pa.Table:
    if page is None:
        return table
    if page < 0:
        raise ValueError("Page numbers must be greater than or equal to zero")
...
class DataHandle:
    ...
    def open(self, format: str, page: int | None = None):
        adapter = _ADAPTER_BY_FORMAT.get(format)
        if adapter is None:
            raise ValueError(f"Unsupported format '{format}'. Expected one of {SUPPORTED_FORMATS}.")
        with adapter.open(self.as_arrow(page)) as payload:
            yield payload
...
class ResponseEnvelope:
    ...
    @property
    def table(self) -> pa.Table:
        """Return the cached rows as a :class:`pyarrow.Table`."""
