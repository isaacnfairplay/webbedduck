...
SUPPORTED_FORMATS: tuple[str, ...] = ("arrow", "parquet", "csv", "json")
...
def _slice_table(table: pa.Table, page_size: int, page: int | None) -> pa.Table:
    if page is None:
        return table
    if page < 0:
        raise ValueError("Page numbers must be zero or greater")
...
class _BaseAdapter:
    """Runtime adapter that materialises a table in a specific format."""

    format: str

    @contextlib.contextmanager
    def open(self, table: pa.Table):  # pragma: no cover - interface definition
...
class DataHandle:
    ...
    _table: pa.Table
    _page_size: int
    _adapters: Mapping[str, _BaseAdapter] = field(init=False, repr=False, compare=False)
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self,
            "_adapters",
            {name: adapter_lookup[name] for name in SUPPORTED_FORMATS},
        )
...
class ResponseEnvelope:
    ...
    @property
    def table(self) -> pa.Table:
        """Convenience accessor for the full result set as an Arrow table."""
